...
void Dijkstra(Graphe *graphe, pSommet *sommet, int D) {
    ...
    while (tous_marques == false) { // tant que nous n'avons pas marqué tous les sommets
        ...
        while (arc != NULL) {
            ...
            if (arc->valeur < dmin &&
                sommet[arc->sommetS]->m == 0) {//si on trouve une valeur plus petite que celle établie, on la remplace
...
int main() {
    ...
    if (rep == 1) {
        ...
        printf("Vous avez choisi l'algorithme de Prim \n");
        printf("Quel est le sommet de depart : \n");
        scanf("%d", &depart);
    } else {
        printf("Vous n'avez pas choisi un algorithme valide \n");
    }
